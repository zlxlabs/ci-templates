...
回执回答的是「本次运行有没有改变生产上跑的东西，改成了什么」，不是「脚本退出码是多少」。
只有进入替换窗口后的 `do_deploy()` 终结路径才写回执；配置校验、锁参数/`flock` 错误、
忙锁 deferred、以及替换窗口前的预拉取失败都已知没有改变正在运行的容器，因此不写回执。
没有回执既不等于部署成功，也不等于部署失败；它的含义是「本次运行没有改变生产上跑的东西」，
具体原因看退出码。消费端不得把回执缺失读成任何一种部署结局。若 SSH 传输在远端结果回传前
以 `rc=255` 中断，远端是否执行仍按传输层不确定规则处理，不能把本地缺少 evidence 当成结论。

| 退出码 | 语义 | 回执 `outcome` | 生产状态 | on-call 处置 |
|---|---|---|---|---|
...
| 其他前向非零码 | pull/tag/compose 等在探针和回滚前失败 | `deploy_failed` | 未发生回滚，生产状态未证实 | 按失败处置并确认主机 |

`deploy_failed` 是前向部署动作失败的诚实结论，不从退出码推断回滚是否发生。因此 `rc=1`
只有在回滚动作实际成功且回滚探针通过时才对应 `rolled_back`；前向动作恰好返回 `1` 时对应
`deploy_failed`，前向 compose 返回 `23` 也必须写 `deploy_failed` 回执。

`rc=1` 的已回滚情形对 on-call 是同一条指引。
...
`build-deploy.yml` 的健康探针通过后，`pull_and_deploy.sh` 仍持有 host flock，在同一
ssh 会话内对账三段事实：本次 `${GIT_SHA}` 镜像 tag 可 inspect、`<image_name>:latest`
与它拥有相同 image ID、以及 `docker compose ps -q --status running` **加上非 oneshot
服务过滤**后找到的至少一个运行容器通过 `docker inspect` 使用该 image ID。
三段中任一不成立，脚本以 **rc=5** 退出（与 rc=1 已回滚 / rc=3 deferred 区分），
workflow 判红并打印 expected / latest / running 的实际值；对账失败**不**自动回滚
（`last_good_tag` 已推进）。`rc=3` 延期、`rc=1` 已回滚和 `rc=4` 的部署不会执行对账。
